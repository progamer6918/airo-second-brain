...
- **Windows Integration**: 
  - `AIRO-SecondBrain-Sync.vbs`: VBScript wrapper used to execute the sync script completely windowlessly.
  - `AIRO-SecondBrain-Sync.ps1`: The target script for manual PowerShell sync trigger.
  - `AIRO-SecondBrain-InstallTask.ps1`: Automated installer to register the task running windowlessly using wscript.exe (runs every 5 minutes and at logon).
