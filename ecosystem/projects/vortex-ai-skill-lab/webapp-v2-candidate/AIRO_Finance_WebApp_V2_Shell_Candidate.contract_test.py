...
checks = {
    ...
    "remediation_quality_reads_active_snapshot": ("snap.quality.forEach" in text or "qList.forEach" in text),
    ...
    "gate32_overview_metrics_mapping": ("metrics: {" in app_script_code or "metrics: metricsArray" in app_script_code),
    "gate32_overview_summary_mapping": "summary: {" in app_script_code and "period_label:" in app_script_code,
    "gate32_spending_backend_mapping": ("spending: snap.spending_intelligence" in app_script_code or "categories: formattedCats" in app_script_code),
    "gate32_accounts_backend_mapping": ("accounts: snap.wallet_snapshot" in app_script_code or "accounts: formattedAccounts" in app_script_code),
    "gate32_quality_backend_mapping": ("quality: snap.data_quality" in app_script_code or "quality: qualityArray" in app_script_code),
    ...
    "gate35a_v2_html_sha256_parity": hashlib.sha256((candidate.parent.parent / "apps-script-live" / "AIRO_Finance_WebDashboard_V2.html").read_bytes()).hexdigest() == hashlib.sha256(candidate.read_bytes()).hexdigest(),
    ...
    "gate38a_stable_v2_route_exists": len(re.findall(r"viewParam === 'dashboard-v2'\s*\)", app_script_code)) == 1,
    "gate38a_stable_v2_route_target": 'HtmlService.createHtmlOutputFromFile("AIRO_Finance_WebDashboard_V2")' in app_script_code,
    "gate38a_test_v2_route_preserved": len(re.findall(r"viewParam === 'dashboard-v2-test'\s*\)", app_script_code)) == 1,
    "gate38a_test_v2_route_target": 'HtmlService.createHtmlOutputFromFile("AIRO_Finance_WebDashboard_V2")' in app_script_code,
    "gate38a_legacy_dashboard_route_preserved": len(re.findall(r"viewParam === 'dashboard'\s*\)", app_script_code)) == 1,
    "gate38a_legacy_route_target": "airoWebDashboardRenderPage_(e)" in app_script_code,
    "gate38a_default_route_unchanged": "Forbidden or unknown GET request" in app_script_code,
    "gate38a_v2_html_unchanged": hashlib.sha256((candidate.parent.parent / "apps-script-live" / "AIRO_Finance_WebDashboard_V2.html").read_bytes()).hexdigest() == "4fcf79006c531f306dce4a74c8b6fac7b8d792c864da3b2fad819dee09f0699a",
    "gate38a_candidate_unchanged": hashlib.sha256(candidate.read_bytes()).hexdigest() == "4fcf79006c531f306dce4a74c8b6fac7b8d792c864da3b2fad819dee09f0699a",
    "gate38a_active_html_unchanged": hashlib.sha256((candidate.parent.parent / "apps-script-live" / "AIRO_Finance_WebDashboard.html").read_bytes()).hexdigest() == "b427db9f0fbeec6bf4b68152c8c5eaa37c664584a33fde60d8f86259b4b67934",
    "gate38a_no_default_cutover": "airoWebDashboardRenderPage_(e)" in app_script_code,
    "gate38a_reachable_write_zero": not any(re.search(r'\b' + fw + r'\b', app_script_code[app_script_code.find('function doGet'):app_script_code.find('function doGet')+800]) for fw in ['setValue', 'setValues', 'appendRow', 'clearContent', 'deleteRow', 'insertRow']),
    "gate37f_overview_categories_backend": "categories: formattedCats" in app_script_code,
    "gate37f_overview_subcategories_backend": "subcategories: formattedSubcats" in app_script_code,
    "gate37f_ringkasan_spending_shared_arrays": "overview: {" in app_script_code and "categories: formattedCats" in app_script_code,
    "gate37f_empty_category_message": "Belum ada data kategori untuk periode ini." in text,
    "gate37f_empty_subcategory_message": "Belum ada data subkategori untuk periode ini." in text,
    "gate37f_wallet_title_removed": 'card("Wallet")' not in text,
    "gate37f_account_name_title": "cardTitle = isUnresolved ? \"Unresolved account\"" in text or "card(cardTitle)" in text,
    "gate37f_unknown_account_normalization": "\"Unresolved account\"" in app_script_code and "\"UNRESOLVED\"" in app_script_code,
    "gate37f_unresolved_not_exact_canonical": "\"Exact canonical account\"" not in text,
    "gate37f_unresolved_quality_warning": "\"Unresolved Accounts\"" in app_script_code,
    "gate37f_no_raw_identity_exposed": not any(x in app_script_code[app_script_code.find('function airoWebDashboardGetClientSnapshot'):app_script_code.find('function airoWebDashboardGetClientSnapshot')+3000] for x in ['rawRow', 'sheetId', 'rowId', 'secretKey']),
    "gate37f_reachable_write_zero": not any(re.search(r'\b' + fw + r'\b', app_script_code[app_script_code.find('function airoWebDashboardGetClientSnapshot'):app_script_code.find('function airoWebDashboardGetClientSnapshot')+3000]) for fw in ['setValue', 'setValues', 'appendRow', 'clearContent', 'deleteRow', 'insertRow', 'setProperty', 'createTrigger', 'sendEmail', 'UrlFetchApp']),
    "gate37a_overview_metrics_array": "Array.isArray(snap.overview.metrics)" in text or "overview: {" in app_script_code,
    "gate37a_spending_collections": "categories: formattedCats" in app_script_code and "top_categories: formattedCats" in app_script_code,
    "gate37a_account_name_mapping": "accTitle = account.name" in text or "name: accName" in app_script_code,
    "gate37a_quality_array_contract": "qualityArray" in app_script_code and "qList" in text,
    "gate37a_provenance_dynamic_update": "updateProvenance" in text,
    "gate37a_candidate_v2_parity": hashlib.sha256((candidate.parent.parent / "apps-script-live" / "AIRO_Finance_WebDashboard_V2.html").read_bytes()).hexdigest() == hashlib.sha256(candidate.read_bytes()).hexdigest(),
