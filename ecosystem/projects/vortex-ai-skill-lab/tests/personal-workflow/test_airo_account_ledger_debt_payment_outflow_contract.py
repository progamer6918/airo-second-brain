...
class AccountLedgerDebtPaymentOutflowContractTest(unittest.TestCase):
    def setUp(self):
        self.source = read_source()

    def test_debt_payment_handler_writes_account_ledger_outflow(self):
        body = extract_function(self.source, "appendDebtPaymentAndUpdateMaster_")

        self.assertIn("writeAccountLedgerMirror_", body)
        self.assertIn("debt_payment", body)
        self.assertRegex(body, r"type\s*:\s*['\"]debt_payment['\"]")
        self.assertRegex(body, r"category\s*:\s*parsed\.category\s*\|\|\s*['\"]Hutang['\"]")
        ...
        self.assertRegex(body, r"writeAccountLedgerMirror_\(ss,\s*accountParsed,\s*rawText,\s*accountCommon,\s*AIRO_CONFIG\.tabs\.hutang\)")

    def test_account_ledger_mirror_treats_debt_payment_as_outflow_type(self):
    ...
    def test_debt_increase_handler_is_not_rewritten_by_payment_patch(self):
        body = extract_function(self.source, "appendDebtIncreaseAndUpdateMaster_")

        self.assertIn("appendDebtPaymentLog_", body)
