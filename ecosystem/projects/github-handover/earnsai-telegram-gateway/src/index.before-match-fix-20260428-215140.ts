...
function classifyInitialRoute(rawText: string): InitialRoute {
  ...
  if (hasAny(["ingatkan", "reminder", "tolong ingatkan", "deadline", "follow up", "todo", "tugas", "kerjakan", "action item"])) {
    return {
      routedTo: "Tasks",
  ...
  if (hasAny(["skripsi", "pai", "penelitian", "tesis", "rumusan masalah", "metode penelitian", "kajian teori"])) {
    return {
      routedTo: "Notes",
