...
def get_btc_price():
    """
    Robust BTC price fetcher.
    Primary: Bybit.
    Fallback: Binance, OKX, then cached/entry price.
    Keeps /status and /balance usable even when one exchange API times out.
    """
    import requests
    ...
    for endpoint in endpoints:
        try:
            ...
            print(f"⚠️ Price feed failed via {endpoint['name']}: {err}")
    ...
    raise RuntimeError("Semua price feed gagal dan belum ada cached price.")
...
def status_command(message):
    try:
        ...
        resp = (
            ...
            f"BTC Feed: {user_data.get('last_price_source', 'unknown')} — {format_money(price)}\n"
