...
RESTORE_CONFIRMATION_TTL_SECONDS = 60
...
def load_trade_log():
    ...
    try:
        ...
        print("⚠️ trade_log.json rusak. Menggunakan trade log kosong sementara.")
        return []


def save_trade_log(records):
...
def create_snapshot_file(message):
    snapshot = {
        "timestamp": datetime.now().isoformat(timespec="seconds"),
        "bot_version": BOT_VERSION,
        "created_by": get_user_id(message),
        "trading_data": dict(user_data),
...
def request_restore_confirmation(message):
    user_id = get_user_id(message)

    if not os.path.exists(SNAPSHOT_FILE):
        bot.reply_to(message, "⚠️ Snapshot belum ada. Jalankan /snapshot dulu.")
        return
    ...
    bot.reply_to(
        ...
        "━━━━━━━━━━━━━━━━━━━━\n"
        "Action: RESTORE PORTFOLIO SNAPSHOT\n"
        f"Expires: {RESTORE_CONFIRMATION_TTL_SECONDS} seconds\n\n"
        "Kirim `/restore_snapshot_confirm` untuk restore.\n"
        "Kirim `/cancel_restore` untuk batal.",
        parse_mode="Markdown"
    )


def consume_restore_confirmation(message):
    user_id = get_user_id(message)
    pending = PENDING_RESTORES.get(user_id)

    if not pending:
        bot.reply_to(
            message,
            "⚠️ Tidak ada pending restore confirmation.\n"
            "Kirim /restore_snapshot dulu, lalu /restore_snapshot_confirm."
        )
        return False

    age = int(time.time()) - int(pending.get("created_at", 0))

    if age > RESTORE_CONFIRMATION_TTL_SECONDS:
        PENDING_RESTORES.pop(user_id, None)
        bot.reply_to(message, "⏳ Pending restore sudah expired. Kirim /restore_snapshot lagi.")
        return False
...
def is_transaction_locked():
    return TRANSACTION_LOCK


def set_transaction_lock(value):
    global TRANSACTION_LOCK
    TRANSACTION_LOCK = value


def reject_if_transaction_locked(message):
    if is_transaction_locked():
        bot.reply_to(
            message,
            "⏳ Sistem sedang memproses transaksi/restore.\n"
            "Tunggu sampai proses selesai, lalu kirim command lagi."
        )
        return True
    return False
...
def multi_command_message(message):
    ...
    for cmd in commands:
        if cmd in ["/status", "/system"]:
            ...
            trades_command(message)
        elif cmd in ["/help", "/start"]:
            start_command(message)
        elif cmd == "/whoami":
            whoami_command(message)
        elif cmd in ["/buy", "/sell", "/buy_confirm", "/sell_confirm", "/snapshot", "/restore_snapshot", "/restore_snapshot_confirm", "/cancel_restore"]:
            bot.reply_to(
                message,
...
def start_command(message):
    resp = (
        ...
        "/balance - Cek portfolio & unrealized P/L\n/trades - Lihat trade journal terakhir\n/snapshot - Simpan snapshot portfolio\n/restore_snapshot - Minta konfirmasi restore snapshot\n"
...
def buy_confirm_command(message):
    ...
    if reject_if_transaction_locked(message):
        return
    ...
    try:
        ...
        log_trade({
            ...
            "price_source": source,
            "btc_qty": qty,
            "balance_usdt_after": user_data["balance_usdt"],
            "balance_btc_after": user_data["balance_btc"],
            "entry_price_after": user_data["entry_price"]
        })
...
def snapshot_command(message):
    ...
    snapshot = create_snapshot_file(message)
    td = snapshot.get("trading_data", {})
    tl = snapshot.get("trade_log", [])
    ...
    resp = (
        ...
        f"USDT: {format_money(float(td.get('balance_usdt') or 0))}\n"
        f"BTC: {float(td.get('balance_btc') or 0):.8f}\n"
        f"Trade Records: {len(tl)}"
    )
    bot.reply_to(message, resp, parse_mode="Markdown")


@bot.message_handler(commands=["restore_snapshot"])
def restore_snapshot_command(message):
    if not require_trade_permission(message):
        return

    request_restore_confirmation(message)


@bot.message_handler(commands=["restore_snapshot_confirm"])
def restore_snapshot_confirm_command(message):
    global user_data
    ...
    if not consume_restore_confirmation(message):
        return
    ...
    if not snapshot:
        bot.reply_to(message, "⚠️ Snapshot tidak valid atau tidak bisa dibaca.")
        return

    restored_data = snapshot.get("trading_data") or {}
    restored_log = snapshot.get("trade_log") or []

    for key, value in DEFAULT_DATA.items():
        if key not in restored_data:
            restored_data[key] = value

    if not isinstance(restored_log, list):
        restored_log = []

    user_data = restored_data
    save_data(user_data)
    save_trade_log(restored_log)

    resp = (
        "✅ *PORTFOLIO SNAPSHOT RESTORED*\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"Snapshot Time: {snapshot.get('timestamp', '-')}\n"
        f"USDT: {format_money(float(user_data.get('balance_usdt') or 0))}\n"
        f"BTC: {float(user_data.get('balance_btc') or 0):.8f}\n"
        f"Trade Records Restored: {len(restored_log)}"
    )
    bot.reply_to(message, resp, parse_mode="Markdown")
    set_transaction_lock(False)
...
def cancel_restore_command(message):
    ...
    if user_id in PENDING_RESTORES:
        PENDING_RESTORES.pop(user_id, None)
        bot.reply_to(message, "✅ Pending restore confirmation dibatalkan.")
    else:
        bot.reply_to(message, "ℹ️ Tidak ada pending restore confirmation.")
...
def trades_command(message):
    ...
    if not records:
        bot.reply_to(
            ...
            "📒 *TRADE JOURNAL*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Belum ada confirmed trade yang tercatat.",
            parse_mode="Markdown"
        )
        return

    last_records = records[-5:][::-1]
    lines = [
        "📒 *TRADE JOURNAL*",
        "━━━━━━━━━━━━━━━━━━━━",
    ...
    for item in last_records:
        ...
        qty = float(item.get("btc_qty") or 0)
        source = item.get("price_source", "-")

        if action == "SELL":
            pnl = float(item.get("pnl_usdt") or 0)
            pnl_pct = float(item.get("pnl_pct") or 0)
            pnl_text = f" | P/L: {format_money(pnl)} ({pnl_pct:.2f}%)"
        else:
            spent = float(item.get("spent_usdt") or 0)
            pnl_text = f" | Spent: {format_money(spent)}"

        lines.append(
            f"- {timestamp} | {action} | {qty:.8f} BTC @ {format_money(price)} | {source}{pnl_text}"
        )
...
def status_command(message):
    try:
        ...
        resp = (
            ...
            f"Trade Log: {'OK' if os.path.exists(TRADE_LOG_FILE) else 'EMPTY/MISSING'}\n"
            f"Snapshot: {'OK' if os.path.exists(SNAPSHOT_FILE) else 'MISSING'}\n"
            f"Transaction Lock: {'ON' if is_transaction_locked() else 'OFF'}\n"
            "Handler Mode: SEQUENTIAL\n"
            ...
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Milestone: v3.1.9 Sequential Handler Mode"
        )
        bot.reply_to(message, resp, parse_mode="Markdown")
